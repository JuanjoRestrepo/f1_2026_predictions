...
5. **Explain decisions** — especially for infra and architecture. Say *why*, not just *what*.

### Quick Stack Decision Guide

**Note:** for questions about system *structure* — microservices vs monolith, DDD, CQRS,
...
Beyond CI/CD pipeline conventions, GitHub repositories require specific hardening at the
workflow, organization, and tooling level:

- **OIDC for cloud auth** — replace all static AWS/GCP/Azure credentials stored as Secrets
  with short-lived tokens issued per workflow run via OpenID Connect; eliminates the
  credential-theft risk documented in GhostAction (Sep 2025) and TanStack (May 2026)
- **GITHUB_TOKEN minimal permissions** — declare `permissions:` at workflow and job level;
  never use `permissions: write-all`
- **Pin actions to full SHA** — `actions/checkout@v4` is mutable; pin to commit SHA and
  keep pins updated via Renovate or Dependabot `github-actions` updates
- **Script injection prevention** — never interpolate `${{ github.event.* }}` directly
  into `run:` commands; pass context values via `env:` variables instead
- **GHAS Secret Protection** — enable push protection to block secrets before they reach
  the remote; custom patterns for organization-specific credential formats
- **GHAS Code Security (CodeQL)** — semantic vulnerability analysis on push and PR;
  Copilot Autofix for automated remediation suggestions
- **Environment protection rules** — require human approval before production deployments
  with environment-scoped secrets revealed only after approval

→ See `references/github-security.md` for full OIDC configuration (AWS/GCP/Azure), IAM
trust policies with sub-claim restrictions, GITHUB_TOKEN permission table, SHA pinning
automation, script injection patterns to avoid, CodeQL workflow setup, dependency review,
organization-level security baseline, and the complete GitHub security checklist.
...
- Commit messages: imperative mood, 72-char subject limit, body explains *what* and *why*
...
- `references/github-security.md` — OIDC for cloud auth (AWS/GCP/Azure, eliminates static
  credentials), GITHUB_TOKEN minimal permissions, SHA pinning with Renovate/Dependabot,
  script injection prevention, GHAS Secret Protection (push protection, custom patterns),
  GHAS Code Security (CodeQL workflow, Copilot Autofix, dependency review), environment
  protection rules, organization-level security baseline, complete GitHub security checklist
