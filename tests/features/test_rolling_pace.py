...
def test_rolling_pace_basic_calculation():
...
def test_rolling_pace_stint_isolation():
    ...
    # Lap 3 is start of Stint 2. Pace should reset to 100.0, not include Stint 1 values.
...
def test_rolling_pace_empty_input():
...
def test_rolling_pace_sorting_handled():
...
def test_delta_to_fastest():
    """Verify the delta_to_fastest_s feature calculation."""
    from f1_predictions.features.rolling_pace import add_lap_delta_to_fastest
    data = {
        "Driver": ["VER"] * 3,
        "Stint": [1] * 3,
