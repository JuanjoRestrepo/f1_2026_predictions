...
def run_residual_analysis(
    ...
    if not pred_path.exists():
        logger.error(
            "Predictions not found for %s %d at %s", event_name, year, pred_path
        )
        return None
    ...
    logger.info(
        "Fetching actual results for %s %d (Round %d)...",
        event_name,
        year,
        round_number,
    )
    ...
    try:
        ...
        logger.exception("Failed to load FastF1 session")
        return None
    ...
    if df_merged.empty:
        logger.error(
            "Could not match any drivers between predictions and actual results."
        )
        return None

    df_merged["residual_s"] = (
        df_merged["actual_median_s"] - df_merged["median_predicted_s"]
    )
    df_merged["abs_error_s"] = df_merged["residual_s"].abs()

    # Coverage check: did the actual time fall within the P05-P95 range?
    df_merged["in_range"] = (
        df_merged["actual_median_s"] >= df_merged["lower_bound_s"]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analyze model residuals after a race."
    )
    ...
    run_residual_analysis(
        year=args.year, round_number=args.round, event_name=args.event
    )
